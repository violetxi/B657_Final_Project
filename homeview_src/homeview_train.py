...
x_train, x_validation, x_test = X.load_imgs(img_source)
# training, validation and testing set
x_train = x_train.astype('float32') / 255.
x_train = x_train.reshape((x_train.shape[0],) + original_img_size)
x_validation = x_validation.astype('float32') / 255.
x_validation = x_validation.reshape((x_validation.shape[0],) + original_img_size)
print("Complete reshaping dataset: Training: {} Validation: {}.".format(x_train.shape, x_validation.shape))
...
vae.save('../models/homeview_ld_%d_conv_%d_id_%d_e_%d_vae.h5' % (latent_dim, num_conv, intermediate_dim, epochs))
encoder.save('../models/homeview_ld_%d_conv_%d_id_%d_e_%d_encoder.h5' % (latent_dim, num_conv, intermediate_dim, epochs))
generator.save('../models/homeview_ld_%d_conv_%d_id_%d_e_%d_generator.h5' % (latent_dim, num_conv, intermediate_dim, epochs))
